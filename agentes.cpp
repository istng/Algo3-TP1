...
void confiablesConPodasAux(std::vector< std::vector<int> > agentes, std::vector<int>& restantes, std::vector<int>& elegidos){
    ...
    if(restantes.size() == 0){
        ...
        imprimirVector(elegidos);
            ...
            if(puedoAgregarloConPoda(agentes, restantes, elegidos, actual)){       //me fijo si puedo agregar o no al agente al conjunto actual
...
/*
    std::cout << std::endl << cantidadAgentes << " " << cantidadVotos << std::endl;
